...
def _aggregate_productivity_from_commits(df: pd.DataFrame) -> pd.DataFrame:
    ...
    lines_changed, issue_refs, conventional_commits, hotfixes
    """
    if df.empty:
        return pd.DataFrame(
            columns=[
...
def create_enhanced_charts(commits_core, prod_core, weekly_trends):
    ...
    charts['top_quality'] = px.bar(
        ...
        y='developer',
        orientation='h',
        title='Top Developers by Average Quality Score (period)',
    ...
    # 4. Daily commits by developer
    # Build true daily counts (not cumulative) with reindex to fill missing days per developer
    ...
    # 5. Repository activity heatmap (Top 10 repos)
    ...
    # 7. Developer summary table (period)
    dev_table_df = period_prod.sort_values(['total_commits', 'avg_quality_score'], ascending=[False, False])
    charts['developer_summary_table'] = _plot_table_from_df(dev_table_df, 'Developer Summary (period)')

    # 8. Repository leaderboard table (period)
    ...
    charts['repo_leaderboard'] = _plot_table_from_df(repo_leader.head(15), 'Top Repositories by Commits')
...
<body>
    ...
    function initDeveloperFilters(tabId) {{
        ...
        var weeklyId = tabId + '-weekly-commits';
